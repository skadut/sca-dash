      ...
      <Card className="border-border/50 bg-card">
        ...
        <CardContent className="space-y-3">
          {recentKeys.map((key, index) => (
            ...
            >
              <div className={`h-12 w-12 rounded-full flex items-center justify-center font-semibold ${getAvatarColor(index)}`}>
                {getInitials(key.nama_aplikasi)}
              </div>
              <div className="flex-1 min-w-0">
                <p className="font-medium text-foreground truncate">{key.nama_aplikasi}</p>
                <p className="text-sm text-muted-foreground">
                  {key.id_login} • {getTimeAgo(key.created_at)}
                </p>
              </div>
              <div className="text-right">
                <p className="font-mono text-sm font-semibold text-foreground">{key.key_id}</p>
            ...
            return (
              ...
              >
                <div className="relative">
                  <div className={`h-12 w-12 rounded-full flex items-center justify-center font-semibold ${getAvatarColor(item.rank - 1)}`}>
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-sm text-muted-foreground">
                    {item.count} {item.count === 1 ? 'key' : 'keys'} generated
                  </p>
                </div>
                <div className="text-right">
                  <p className="font-mono text-lg font-semibold text-foreground">{item.count}</p>
